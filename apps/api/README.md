...
Backend NestJS com integração WhatsApp (Evolution API) e IA com function calling.
O provider de LLM é configurável via `AI_BACKEND`: padrão atual **Ollama** (local);
`anthropic` (Claude) e `openai-compatible` também disponíveis sem mudar a lógica de negócio.
...
│   ├── orders/          Pedidos
│   ├── ai/              Agente + function calling no catálogo
...
O `AiService` roda um loop: o modelo pode chamar tools (`search_products`,
`check_product_availability`, `list_categories`, `request_human_handoff`) até
responder. As tools estão em `ai/catalog.tools.ts` e consultam o banco em tempo
real com scope por tenant.

O provider ativo é escolhido pela `LlmProviderFactory` a partir de `AI_BACKEND`
(default `ollama`). Para usar Claude: `AI_BACKEND=anthropic` + `ANTHROPIC_API_KEY`.
